...
namespace MyShop.Services
{
    public class ProductService
    {
        ...
        public void Add(Product product,bool isSaveChanges)
        {
        ...
        public void Update(Product product,bool isSave=true)
        {
            var srt = _context.Products.FirstOrDefault(p => p.Id == product.Id);
            srt.Price = product.Price;
            srt.Description = product.Description;
            srt.Category = product.Category;
            srt.Discount = product.Discount;
            srt.Name = product.Name;
            srt.CategoryId = product.CategoryId;
            srt.VendorId = product.VendorId;
            if (isSave)
            {
                _context.SaveChanges();
            }
        ...
        {
            var srt = _context.Products.FirstOrDefault(p=>p.Id == Id);
            srt.IsDeleted = true;
            _context.SaveChanges();
        }
        public List<Product> GetByFilter(ProductFilterModel filter)
        {
            ...
            && (filter.Name == null || p.Name.ToLower().Contains(filter.Name.ToLower()))).ToList();
            return products;
